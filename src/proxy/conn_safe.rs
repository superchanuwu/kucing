...
impl<'a> ProxyStream<'a> {
    async fn process_dns_query(&mut self) -> Result<()> {
        let mut buf = vec![0u8; 512];
        let n = self.read(&mut buf).await?;
        
        if n <= 1 {
            return Err(Error::RustError("DNS payload too short".into()));
        }

        let req = &buf[1..n]; // skip 0xF1 prefix

        console_log!("DNS request (hex): {}", hex::encode(req));
        match crate::dns::doh(req).await {
            Ok(response) => {
                console_log!("DNS response length: {}", response.len());
                self.write(&response).await?;
            }
            Err(e) => {
                console_error!("DNS error: {}", e);
            }
        }
    ...
    pub async fn process(&mut self) -> Result<()> {
        ...
        if peeked_buffer.len() < 58 {
            return Err(Error::RustError("peeked buffer too short".into()));
        }
    ...
    pub async fn handle_tcp_outbound(&mut self, addr: String, port: u16) -> Result<()> {
        ...
        let ip = crate::dns::resolve_doh(&addr)
            .await
    ...
    pub async fn handle_udp_outbound(&mut self) -> Result<()> {
        ...
        match crate::dns::doh(data).await {
            ...
            Err(e) => {
                console_error!("DNS resolve error: {}", e);
            }
        }
        Ok(())
    }
