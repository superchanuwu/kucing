
use crate::config::Config;
use reqwest::Client;
use tokio::io::{AsyncReadExt, AsyncWriteExt, ReadBuf};
use bytes::{BufMut, BytesMut};
use futures_util::Stream;
use pin_project_lite::pin_project;
use worker::*;
use std::pin::Pin;
use std::task::{Context, Poll};
use crate::dns::{resolve_doh};  // Import DNS functions

pin_project! {
    pub struct ProxyStream<'a> {
        pub config: Config,
        pub ws: &'a WebSocket,
        pub buffer: BytesMut,
        #[pin]
        pub events: EventStream<'a>,
    }
}

impl<'a> ProxyStream<'a> {
    pub fn new(config: Config, ws: &'a WebSocket, events: EventStream<'a>) -> Self {
        let buffer = BytesMut::new();

        Self {
            config,
            ws,
            buffer,
            events,
        }
    }

    pub async fn fill_buffer_until(&mut self, n: usize) -> std::io::Result<()> {
        use futures_util::StreamExt;

        while self.buffer.len() < n {
            match self.events.next().await {
                Some(Ok(WebsocketEvent::Message(msg))) => {
                    if let Some(data) = msg.bytes() {
                        self.buffer.put_slice(&data);
                    }
                }
                Some(Ok(WebsocketEvent::Close(_))) => {
                    break;
                }
                Some(Err(e)) => {
                    return Err(std::io::Error::new(std::io::ErrorKind::Other, e.to_string()));
                }
                None => {
                    break;
                }
            }
        }

        Ok(())
    }

    pub fn peek_buffer(&self, n: usize) -> &[u8] {
        let len = self.buffer.len().min(n);
        &self.buffer[..len]
    }

    pub async fn process(&mut self) -> Result<()> {
        self.fill_buffer_until(62).await?;
        let peeked_buffer = self.peek_buffer(62);

        // Periksa HTTP/HTTPS Request
        if peeked_buffer.starts_with(b"GET") || peeked_buffer.starts_with(b"POST") || peeked_buffer.starts_with(b"CONNECT") {
            // Dapatkan domain dari HTTP request
            let target_url = self.extract_url_from_request(peeked_buffer).await;
            return self.handle_http_request(target_url).await;
        }

        // Proses protokol lainnya (VLESS, Shadowsocks, Trojan, VMess)
        if peeked_buffer[0] == 0 {
            console_log!("VLESS detected!");
            self.process_vless().await
        } else if peeked_buffer[0] == 1 || peeked_buffer[0] == 3 {
            console_log!("Shadowsocks detected!");
            self.process_shadowsocks().await
        } else if peeked_buffer[56] == 13 && peeked_buffer[57] == 10 {
            console_log!("Trojan detected!");
            self.process_trojan().await
        } else {
            console_log!("Vmess detected!");
            self.process_vmess().await
        }
    }

    // Fungsi untuk mengekstrak URL dari HTTP/HTTPS request (misalnya youtube.com, facebook.com)
    async fn extract_url_from_request(&self, buffer: &[u8]) -> String {
        let request_str = std::str::from_utf8(buffer).unwrap_or("");
        let url_start = request_str.find("Host: ").map(|start| start + 6);
        if let Some(start) = url_start {
            let url_end = request_str[start..].find("
").unwrap_or(0);
            return request_str[start..start + url_end].to_string();
        }
        "unknown".to_string()  // Default jika URL tidak ditemukan
    }

    // Fungsi untuk menangani HTTP/HTTPS request dan relay response dengan chunking
    pub async fn handle_http_request(&mut self, addr: String) -> Result<()> {
        let mut client = Client::new();
        
        // Membaca data secara chunk (misalnya 8 KB)
        let mut request = Vec::new();
        let mut buffer = vec![0u8; 8192]; // 8KB buffer size
        while let Ok(n) = self.read(&mut buffer).await {
            if n == 0 {
                break;
            }
            request.extend_from_slice(&buffer[..n]);
        }

        // Menggunakan `reqwest` untuk forward request ke server tujuan
        let response = client
            .get(&addr)
            .body(request)
            .send()
            .await?;

        // Menulis data response secara chunk juga
        let mut body = response.bytes_stream();
        while let Some(chunk) = body.next().await {
            let chunk = chunk?;
            self.write_all(&chunk).await?;
        }

        Ok(())
    }

    // Fungsi untuk menangani UDP traffic untuk game online
    pub async fn handle_udp_traffic(&mut self, addr: String, port: u16) -> Result<()> {
        console_log!("Resolving {}:{} via DoH...", addr, port);
        let ip = resolve_doh(&addr)
            .await
            .map_err(|e| Error::RustError(format!("DNS resolve failed: {}", e)))?;

        console_log!("Connecting to upstream {}:{}", ip, port);
        let mut remote_socket = Socket::builder()
            .connect(ip.to_string(), port)
            .map_err(|e| Error::RustError(e.to_string()))?;

        remote_socket
            .opened()
            .await
            .map_err(|e| Error::RustError(e.to_string()))?;

        // Relay UDP traffic between WebSocket and game server
        tokio::io::copy_bidirectional(self, &mut remote_socket)
            .await
            .map_err(|e| Error::RustError(e.to_string()))?;

        Ok(())
    }

    pub async fn handle_tcp_outbound(&mut self, addr: String, port: u16) -> Result<()> {
        console_log!("Resolving {}:{} via DoH...", addr, port);
        let ip = resolve_doh(&addr)
            .await
            .map_err(|e| Error::RustError(format!("DNS resolve failed: {}", e)))?;

        console_log!("Connecting to upstream {}:{}", ip, port);
        let mut remote_socket = Socket::builder()
            .connect(ip.to_string(), port)
            .map_err(|e| Error::RustError(e.to_string()))?;

        remote_socket
            .opened()
            .await
            .map_err(|e| Error::RustError(e.to_string()))?;

        tokio::io::copy_bidirectional(self, &mut remote_socket)
            .await
            .map_err(|e| Error::RustError(e.to_string()))?;

        Ok(())
    }

    pub async fn handle_udp_outbound(&mut self) -> Result<()> {
        let mut buff = vec![0u8; 65535];

        let n = self.read(&mut buff).await?;
        let data = &buff[..n];
        if crate::dns::doh(data).await.is_ok() {
            self.write(&data).await?;
        };
        Ok(())
    }
}

impl<'a> AsyncRead for ProxyStream<'a> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &mut ReadBuf<'_>,
    ) -> Poll<tokio::io::Result<()>> {
        let mut this = self.project();

        loop {
            let size = std::cmp::min(this.buffer.len(), buf.remaining());
            if size > 0 {
                buf.put_slice(&this.buffer.split_to(size));
                return Poll::Ready(Ok(()));
            }

            match this.events.as_mut().poll_next(cx) {
                Poll::Ready(Some(Ok(WebsocketEvent::Message(msg)))) => {
                    msg.bytes().iter().for_each(|x| this.buffer.put_slice(&x));
                }
                Poll::Pending => return Poll::Pending,
                _ => return Poll::Ready(Ok(())),
            }
        }
    }
}

impl<'a> AsyncWrite for ProxyStream<'a> {
    fn poll_write(
        self: Pin<&mut Self>,
        _: &mut Context<'_>,
        buf: &[u8],
    ) -> Poll<tokio::io::Result<usize>> {
        return Poll::Ready(
            self.ws
                .send_with_bytes(buf)
                .map(|_| buf.len())
                .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string())),
        );
    }

    fn poll_flush(self: Pin<&mut Self>, _: &mut Context<'_>) -> Poll<tokio::io::Result<()>> {
        Poll::Ready(Ok(()))
    }

    fn poll_shutdown(self: Pin<&mut Self>, _: &mut Context<'_>) -> Poll<tokio::io::Result<()>> {
        unimplemented!()
    }
}
